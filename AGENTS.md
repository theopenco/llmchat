...
- **Checkout / portal**: `POST /billing/checkout` (owner-only) creates/reuses a Stripe customer and a Checkout session with the base price + optional **metered overage** price; the dashboard redirects with `@stripe/stripe-js` (`NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY`). `POST /billing/portal` opens the Stripe billing portal. New subscriptions (workspace not already on a paid plan) get a **7-day free trial** via `subscription_data[trial_period_days]` — card collected upfront, `trialing` treated as entitled; `TRIAL_PERIOD_DAYS` in `@llmchat/shared` is the single source of truth for the length and every UI mention (see `docs/billing-free-trial.md`).
