...
- Top-level: `kind` (`worker` | `dynamic` | `nextjs` | `static`), `name`, `build`, `out`, `main`, `base`, `dev: { port?, host? }`, `compatibility_date`, `compatibility_flags`, `agentSDK`, `ai`, `assets: { directory?, binding?, not_found_handling?, run_worker_first? }` (`directory` is what makes a dynamic simple-worker deploy upload static assets — see `apps/shopify/ploy.yaml`).
- Bindings (each is a binding-name → resource-name map; binding names UPPER_SNAKE, resource names lower_snake): `db`, `state`, `queue`, `workflow`, `cron`, `timer`, `fs`. There is **no `kv:` field** — KV is `state:`. There is **no `routes:` or `secrets:` field** — domains are dashboard-managed.
- **There is no `env:` field.** As of `@meetploy/cli@1.46`, an `env:` block in a project `ploy.yaml` is a hard error ("`'env' in <file>` is no longer supported"). Env vars are **not declared** anywhere: every variable set in the Ploy **project settings** is available as `env.vars.*` at runtime, and a local `.env` is used during development. Don't re-add the block — just read the variable. (`ploy-workspace.yaml` still accepts `env`.) The public build-time `NEXT_PUBLIC_*` literals the Next apps used to declare there now live in a committed per-app `.env.production` (un-ignored in `.gitignore`); a real var set in project settings overrides that file at build time. **Consequence — don't run `ploy types` in `apps/api`:** it derived `PloyEnv["vars"]` from the (now absent) `env:` block, so it now emits `vars: Record<string, string>`, throwing away the per-variable types behind the ~66 `env.vars.*` reads in `src/` (and breaking `tsc --noEmit` on the partial-env cast in `src/lib/request.test.ts`). `apps/api/env.d.ts` is hand-maintained now; add new variables to it and to `.env.example` by hand. It also rewrites the Next apps' `tsconfig.json` + drops an `env.d.ts` in each, so `git checkout` after any accidental run.
...
`apps/api/ploy.yaml` binds `DB → llmchat_db`, `STATE → llmchat_state` and two `cron:` triggers (the weekly/monthly traffic report — expressions must stay in sync with `TRAFFIC_CRON_*` in `src/lib/traffic-report.ts`). Its `build:` builds the widget **before** the api (`pnpm --filter @llmchat/widget build && pnpm --filter @llmchat/api build`) so `/widget.js` is fresh. Its env (LLM Gateway, Better Auth + OAuth, Resend, Stripe price ids, PostHog, Discord webhooks) is **undeclared** — set in the Ploy project settings for prod, read from `apps/api/.env` in dev; `apps/api/.env.example` is the canonical list. The Next.js apps declare no bindings at all.
...
Local env: `cp apps/api/.env.example apps/api/.env` and fill in keys. `ploy dev` loads that `.env` straight into the worker — nothing is declared in `ploy.yaml` anymore (see the `env:` note above). In production the same variables come from the Ploy project settings. The Next apps read their own `.env`/`.env.example` (`NEXT_PUBLIC_*` only), plus a committed `.env.production` for the public prod literals.
...
- **Checkout / portal**: `POST /billing/checkout` (owner-only) creates/reuses a Stripe customer and a Checkout session with the base price + optional **metered overage** price; the dashboard redirects with `@stripe/stripe-js` (`NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY`). `POST /billing/portal` opens the Stripe billing portal. There is **no free trial** — the card is collected and charged at Checkout. The 7-day trial was removed on 2026-09-19 after it was farmed for free Scale access (one user, a new workspace every 7 days; each workspace mints its own Stripe customer, so Stripe never saw a repeat). `trialing` is still treated as entitled, for trials an operator grants by hand in Stripe and for subscriptions already trialing at removal. `user.trial_started_at` (migration `0030`) is vestigial — applied in prod, never read. See `docs/billing-free-trial.md`; **re-gate per person, not per workspace, if it is ever revived**.
...
A Vite IIFE lib (`formats: ["iife"]`, `inlineDynamicImports: true`, `cssCodeSplit: false`) — a single self-contained `widget.js` mounted into a **shadow DOM**. `mount.tsx` is the IIFE entry: it reads config from the host `<script>`'s data attributes (`data-project`, `data-api`, `data-brand`, `data-mode`, `data-theme`, `data-teaser`, `data-escalation-threshold`; api URL falls back to the origin that served `widget.js`). Bundles `@ai-sdk/react` + `ai` + `react`/`react-dom` + `streamdown` (markdown rendering; `mermaid` is aliased to a stub to keep size down). Features: streaming chat, proactive teaser bubbles above the closed launcher (welcomeMessage lines → bubbles, session-consumed, `data-teaser="off"` opt-out — `teaser.ts`), escalate-to-human, per-message thumbs, CSAT (1–5) on close, message polling/sync via `/v1/messages`, and Scale-only live voice calls whose transcript (assembled client-side from realtime transcript events, `TranscriptRecorder` in `voice-call.ts`) is persisted at call end via `POST /v1/voice/transcript` as a single `system` row on the conversation (find-or-create, `reserveOnce`-deduped per callId, entitlement-gated).
...
Public product docs / knowledge base built with **Fumadocs**, modeled on llmgateway's docs app. `kind: nextjs`, dev port **3005**. Runs **Next.js 16** + `fumadocs-ui`/`fumadocs-core` 16.x + `fumadocs-mdx` (fumadocs 16 requires Next 16 — this app pins its own Next major; every other app stays on Next 15). Content lives in `apps/docs/content/**/*.mdx` (frontmatter `title`/`description`/`icon` + `meta.json` ordering: a top-level Welcome + Getting started, and a `learn/` "Knowledge base" section with one page per dashboard page). Docs render at `/` via a catch-all (`app/(docs)/[[...slug]]/page.tsx`); search is fumadocs' built-in (`app/api/search/route.ts`). Brand: indigo `--color-fd-primary` override in `app/global.css` (Tailwind v4 via `@tailwindcss/postcss` — docs-only; other apps use Tailwind 3). `.source/` is generated by `fumadocs-mdx` (runs on postinstall) and gitignored.

**Screenshots**: every dashboard page is captured in **light and dark** at 1440×900@2x into `apps/docs/public/learn/<name>-{light,dark}.png` and embedded via the global `<ThemedImage>` MDX component (`components/themed-image.tsx` — swaps on `dark:` classes, no import needed in MDX). To regenerate: boot the seeded local stack (`pnpm dev` + `pnpm seed`), run `node apps/docs/scripts/demo-data.mjs` (idempotent-ish realistic inbox fixtures: escalated / visitor-resolved / rated / tagged conversations, knowledge sources), then `pnpm --filter @llmchat/docs exec playwright install chromium` once and `node apps/docs/scripts/capture-screenshots.mjs` (discovers conversation ids via the API; see header comments).
