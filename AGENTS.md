...
- **Privacy:** no PII in event properties. EU/EEA + UK visitors get a cookie-consent gate before `posthog.init` (marketing + dashboard `PostHogProvider` → `ConsentBanner`); region is detected from the browser time zone and the decision stored in `localStorage`. Consent logic is shared in `@llmchat/shared` (`isConsentRequiredRegion`, `getStoredConsent`, `setStoredConsent`); outside those regions analytics loads on implied consent.
