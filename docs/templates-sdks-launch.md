...
3. 🔑 **Hosted demos** (still open): deploy each template (`vercel --cwd templates/<name>` with the public dogfood embed key `pk_adad…` — the one the marketing site's own bubble uses (apps/marketing/src/app/layout.tsx); the old showcase key is dead with the surface — or the Railway template for Laravel), then set `demoUrl` in the `/templates` page's `TEMPLATES` array and redeploy. The link slot lights up automatically.
