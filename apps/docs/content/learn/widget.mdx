...
- **Calls are budgeted.** Voice sessions are limited per visitor (4 calls/hour), per project (20/day), and per workspace (100/day), and each session is capped in duration and spend. If a visitor hits a limit, the widget falls back to the written chat.
