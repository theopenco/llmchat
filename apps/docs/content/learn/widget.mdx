...
| **Agent photo**        | A photo or logo shown on the launcher bubble and the chat header, so the widget carries your team's face. Upload one (it's downscaled and stored with your project) or paste an image URL; an uploaded photo takes precedence. Remove both for the default icon |
