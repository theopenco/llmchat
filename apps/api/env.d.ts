...
// .env in dev). `ploy types` derived the `vars` shape below from that block,
// so with the block gone it now emits `vars: Record<string, string>` instead
// (verified against @meetploy/cli 1.46). Regenerating therefore throws away
// every per-variable type: typos, wrong names, and vars the code treats as
// optional all silently become `string`. It also breaks `tsc --noEmit`
// outright — src/lib/request.test.ts casts a partial env to PloyEnv and that
// cast stops overlapping once `vars` is an index signature.
//
// Don't regenerate this file. Add new variables to the `vars` block below by
// hand (and to apps/api/.env.example), keeping optional (`?`) for anything the
// code tolerates being unset.
